...
func TestMapReduce_1(t *testing.T) {

	val, err := MapReduce(func(source chan<- rsp) {
		// generator
		for i := 0; i < 10; i++ {
			source <- rsp{Id: i, Name: fmt.Sprintf("name_%d", i)}
			t.Logf("generator %d \n", i)
		}
	}, func(i rsp, writer Writer[rsp], cancel func(error)) {
		// mapper
		t.Logf("mapper %+v \n", i)
		writer.Write(i)
	}, func(pipe <-chan rsp, writer Writer[rsps], cancel func(error)) {
		// reducer
		vals := rsps{
			List: make([]rsp, 0),
		}
