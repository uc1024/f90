...
func dumpGoroutines(ctor creator) {
	...
	slogx.Default.Infof(context.Background(), "Got dump goroutine signal, printing goroutine profile to %s", dumpFile)

	if f, err := ctor.Create(dumpFile); err != nil {
		slogx.Default.Errorf(context.Background(), "Failed to dump goroutine profile, error: %v", err)
	} else {
		defer f.Close()
