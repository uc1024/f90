...
func gracefulStop(signals chan os.Signal, sig syscall.Signal) {
	...
	slogx.Default.Infof(context.Background(), "Got signal %d, shutting down...", sig)
	...
	slogx.Default.Infof(context.Background(), "Still alive after %v, going to force kill the process...", delayTimeBeforeForceQuit)
...
func (lm *listenerManager) notifyListeners() {
	...
	group := threadingx.NewRoutineGroup()
